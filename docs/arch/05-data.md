...
- `~/.forge/sessions/abc123.events.jsonl` — transparency events, each stamped with its turn number, so the UI can replay them after reload (same skip-and-count guard).
